...
def check_keywords(infile, template, verbose=True):
    """
...
if __name__ == '__main__':
    """
    @brief Check that FITS files have all of the header keywords
    specified in LCA-10140.
    """
    import sys
    import argparse
    ...
    parser.add_argument('-t', '--template', type=str, default='fits_header_template_=.txt',
                        help='template file for FITS extensions and header keywords')
    args = parser.parse_args()

    missing = check_keywords(args.infile, template=args.template)
    noao_defects = check_noao_keywords(args.infile)
